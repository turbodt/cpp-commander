...
Command *Command::initialize_wrapper() {
  ...
  std::for_each(this->subcommands->begin(), this->subcommands->end(),
                [&](auto it) { it.second->initialize_wrapper(); });
  return this;
}
...
Command *Command::parse_arguments(ArgumentList *list) {
  ...
  while (!list->is_finished() && !exit) {
    ...
    if (argument[0] == '-') {
      ...
      // positional argument
      this->positional_arguments->at(positional_count).set_value(argument);
  ...
  if (positional_count < this->positional_arguments->size() &&
