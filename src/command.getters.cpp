...
Command const *Command::get_parent() const {
    return this->parent;
}
...
PositionalArgument const * Command::get_global_positional(
    ...
    throw NotFoundException(
        "Positional argument with name `" + name + "` was not found."
    );
}
...
PositionalArgument const * Command::get_positional(
    std::string const &name
) const {
...
std::vector<PositionalArgument> Command::get_positionals() const {
    auto positional_arguments = std::vector<PositionalArgument>(
        this->positional_arguments->begin(),
...
Option const *Command::get_global_option(const std::string &name) const {
    ...
    throw NotFoundException(
        "Option with name `" + name + "` was not globally found."
    );
};
...
std::vector<Option *> *Command::get_options() {
    return &this->options;
}
