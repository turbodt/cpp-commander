...
Command::~Command() {
    std::for_each(
        this->self_stored_options.begin(),
...
Command *Command::add_positional(PositionalArgument &positional_argument) {
    if (
        !positional_argument.is_optional()
        && this->positional_arguments->size()
        && this->positional_arguments->back().is_optional()
    ) {
        throw InvalidAdditionException(
            "To add a positional argument after an optional positional"
            " argument is not allowed. (Invalid command label: `"
        ...
        throw InvalidAdditionException(
            "To add an optional positional argument and a"
...
Command *Command::add_positional(
...
Command *Command::add(std::string const &label) {
    ...
    throw InvalidLabelException(
        "Label `" + label + "` is neither"
        " a valid positional argument or option label."
    );
};
...
void Command::parse_positional(int index, ArgumentList *list) {
    list->next();
}
