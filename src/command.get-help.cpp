...
std::string Command::get_help() const {
    ...
    if (this->get_options()->size()) {
        int name_max_size = get_help::get_options_name_max_width(
            this->get_options()
        );
    ...
    if (this->subcommands->size()) {
        int subcommand_max_size = get_help::get_subcommands_label_max_width(
            this->subcommands
        );
    ...
    if (this->get_options()->size()) {
        ss << "\n\nOptions:\n";
        std::for_each(
            this->get_options()->begin(),
            this->get_options()->end(),
            [&](auto option) {
                ss << "\n  " << std::setfill(' ')
