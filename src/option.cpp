...
const std::regex Option::name_regex =
    std::regex("^((\\s*(-[" + accepted_chars_for_labels + "]+)\\s*,?)+)( (" +
               positional_argument_regex_str + ")*)?\\s*$");

const std::regex Option::args_regex =
    std::regex("^(\\s*(-[" + accepted_chars_for_labels + "]+)\\s*,?)+( (" +
               positional_argument_regex_str + ")*)?\\s*$");
...
void Option::on_creation(std::string const &label) { this->parse_label(label); }
...
void Option::parse_label(std::string label) {
  if (!Option::is_option_label(label)) {
    throw InvalidLabelException("Label `" + label +
                                "` is not a valid option label.");
  }
...
std::vector<std::string> *Option::get_names() { return &this->names; }

std::vector<PositionalArgument> const *Option::get_values() const {
  return &this->values;
}

std::vector<PositionalArgument> *Option::get_values() { return &this->values; }
...
const bool Option::has_appeared() const { return this->has_been_parsed; };
